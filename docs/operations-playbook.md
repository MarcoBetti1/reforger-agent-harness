...
2. Open that world directly with `workbench:open --authorize-local-test-scripts --execute`; reacquire the actual World Editor window. The helper's `-run` may auto-start gameplay. Watch the current `.cache/workbench-gui-runs/<run>/script.log` for GAME and a probe start marker and inspect the viewport before any keypress. In a live test, GAME began about eight seconds after launch. Press F5 only if startup settles in edit mode without a preview; another F5 during the auto-preview can restart it and create duplicate start markers.
