...
A fresh September 26 Notepad comparison reproduced the activation boundary outside the game. **Add New Tab** returned `coordinate input geometry is unavailable`; a fresh capture showed the on-screen keyboard covering the upper Notepad window. Ctrl+N then failed with `failed to activate captured window`, as did one retry after fresh window enumeration/selection. No tab was created and no text was sent; the existing document was unchanged. This does not show rejected keyboard delivery because activation failed first. The overlay was observed, but its causal role remains unproved. The next supported positive control is a visible `press_key` character and deletion in a verified empty editor after activation succeeds. Keep that OS-to-app test separate from an engine-action calibration, which bypasses external key delivery.
...
A later probe exposed timer precision: a callback scheduled for 30 seconds observed only **29999.8 ms** on the world clock. Its one-shot minimum-delay check refused closure without rescheduling, leaving the process alive. Preserve that failed lifecycle. A bounded correction allows one 100 ms deferral for a shortfall no greater than 100 ms, then repeats the original identity/world/offline/terminal guards and full minimum delay. Later trials closed naturally after approximately 30.016 seconds; they did **not** exercise the early-deferral branch. Do not infer branch coverage from compilation or ordinary closure. Native cleanup still emitted resource diagnostics, which remain separate from process-exit success.

### Calibrating engine actions separately from desktop input

A private character-controller probe can observe the callback-provided `ActionManager` before and after native `OnPrepareControls`, then pair read-only pre-/post-physics observations. Keep this investigation in the addon fixture. A failed readiness guard before any write is not evidence that a delivered action was ignored. Log individual predicates: exact player/pilot identity, movement/view flags, menus/editor, context activity, action activity, and values. In a saved bounded trial, `CarThrust` was active and neutral while `CarContext` was reported inactive in both callback and global managers; all other readiness checks passed. The fixture wrote no thrust, so powered movement and ordinary input remain unproved. Establish the installed API's timing contract before treating those two activity getters as interchangeable. Never silently force contexts or weaken physical progress criteria to make a calibration pass.

### Further engine-action observations (September 26)

A later private callback-owned action experiment physically powered the original AI-inactive player truck uphill. Action activity and exact callback ownership were checked separately from the previously inactive context getter; no context was forced. This is evidence for an engine-action fixture, not desktop key delivery, AI-driver callback availability, steering, braking, panel clicks or restored human controls. Preserve those separate gates.

Measurement matters: entity `GetOrigin()` remained identical within observed PRE/POST pairs despite motion between them. Fresh local `Physics.GetDirectWorldTransform` observations exposed body movement. The engine warns against storing `Physics` pointers as script members, so retain numeric observations and reacquire a local body each time. Original entity continuity does not prove physics-body generation continuity. Credit only consecutive completed intervals in the same powered phase, with original pilot/entity and valid controls at both ends; sparse diagnostic samples do not independently reconstruct aggregate counters.

Read-only active-state events subsequently showed an unfinished neutral-baseline PRE coinciding with the body becoming inactive, followed by a wake at drive start. Any prospective classification must require the exact observed neutral stationary sleep event, retain raw interruption counts, award no pair/motion credit, and fail unexplained or powered-phase gaps. Keep earlier failed contracts failed; do not infer that every missing callback is a harmless sleep boundary.

The isolated action fixture finally exercised the early-close branch: the first callback observed 29999.3 ms, deferred once by 100 ms, rechecked all guards and requested native exit at 30.0993 seconds. Cleanup and natural process exit followed. This covers that fixture's branch only, not similarly written code in other probes, and retained shutdown diagnostics remain failures under their declared strict gates.
...
## Ordinary input activation boundary — September 26

Supported window capture continued to work while window activation failed. In the observed session, Notepad's `Raise` action and a fresh list/get-window/activate recovery both returned `failed to activate captured window`. Launching Notepad through the supported app API returned its existing document; it did not establish focus. A separately launched Calculator, previously reported not running, failed activation and the same single fresh-window recovery too. No calibration key was sent. These are activation failures, not measured rejection of a delivered game key.

An on-screen keyboard overlay remained visible. Its UI close attempt returned without removing the window; a normal CLI stop of that identified process reported access denied. Its causal role is unproved. No privilege or security changes were attempted. Avoid repeating the same input matrix without an environment change or a different supported method. Preserve existing editor documents, and establish a positive ordinary-key control in a disposable app before interpreting negative game input. In Reforger, a persistent map action event plus native map opening is stronger receipt evidence than a short throttle tap. Engine action-value injection is a separate calibration and does not prove ordinary desktop input or restored driving controls.

A later fresh supported OSK accessibility observation, after the game and recorder had closed, explicitly reported that the target had higher Windows integrity than the Computer Use helper. It exposed only window element 0 with secondary action `Raise` and titlebar element 1; there was no returned Close action. No input was attempted. A distinct accessibility Close method therefore was unavailable in that observation. Do not invent action labels or treat capture as input access; preserve the activation blocker and unproved overlay causation without privilege/security workarounds.

### Dedicated join: next discriminating check (September 26, not run)

The recorded vanilla failures used the helper's raw `-client 127.0.0.1` path. Installed 1.8.0.13 `ServerBrowserMenuUI.c` instead routes ordinary Direct Connect through authenticated backend discovery, `SearchTarget`, `Room.Join`, and only its success callback's `RequestConnectViaRoom`. This difference is a testable hypothesis, not a proven cause of the timeout. The next comparison should retain the same hidden loopback server and security settings, omit `--connect-local`, and use stock Direct Connect with that fresh server's Direct Join Code after normal backend authentication. Require joined-player evidence from both client and server. A blocked splash or input failure before dispatch is not a network result. Preserve the failed address controls; repeating them without changing the join entry would not test this hypothesis. [Bohemia startup parameters](https://community.bistudio.com/wiki/Arma_Reforger%3AStartup_Parameters)
