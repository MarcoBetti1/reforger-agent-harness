...
`--authorize-local-test-scripts` opts a **single Workbench process** into its supported `-scriptAuthorizeAll` flag. This avoids the local script-refresh authorization dialog during unattended previews; omit it for untrusted addon source. A World Editor launch includes `-run`; in a live test it started the selected world automatically about eight seconds after launch. Reacquire the editor window, check the fresh log and view, and press F5 only if no preview started. An extra F5 can restart the active preview and duplicate start markers. Stop gameplay before modifying scripts. A `GAME` log transition or a successful package alone does not prove an in-world feature works.
